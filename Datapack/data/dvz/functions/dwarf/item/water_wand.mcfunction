...
execute if entity @s[scores={dvz.water_wand.cool=1..}] run title @s actionbar [ \
  "", \
  {"text":"[Water Wand]","bold":true,"color":"dark_red"}, \
  {"text":" You have ","color":"dark_red"}, \
  {"score":{"name":"@s","objective":"dvz.water_wand.cool"},"bold":true,"color":"dark_red"}, \
  {"text":" seconds remaining!","color":"dark_red"} \
]
execute if entity @s[scores={dvz.water_wand.cool=1..}] run return 0
...
execute as @s at @e[type=minecraft:marker,tag=ray,limit=1,sort=nearest] if block ~ ~ ~ minecraft:water_cauldron run scoreboard players set @s dvz.water_wand.cool 5
