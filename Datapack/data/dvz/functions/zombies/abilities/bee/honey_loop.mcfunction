...
# File Name: honey_loop
# Function Name: dvz:zombies/bee/honey_loop
# File Purpose: Ray scanning for Bee's overheal ray.
...
execute unless entity @a[tag=temp_ray_origin,distance=..45] run return 0
execute if entity @a[tag=temp_hit] run return 0

# Teleports the ray slightly forward.
tp @s ^ ^ ^0.1

execute if entity @p[tag=temp_ray_origin,distance=2..] run particle minecraft:wax_on ~ ~ ~ 0 0 0 0 1 force
...
execute as @s at @s run function dvz:zombies/bee/honey_loop