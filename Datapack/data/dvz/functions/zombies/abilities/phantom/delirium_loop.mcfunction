...
# File Name: delirium_loop
# Function Name: dvz:zombies/phantom/delirium_loop
# File Purpose: Ray scanning for Phantom's delirium.
...
execute unless entity @a[tag=temp_ray_origin,distance=..12] run return 0
execute if entity @a[tag=temp_hit] run return 0

# Teleports the ray slightly forward.
tp @s ^ ^ ^0.1

execute if entity @p[tag=temp_ray_origin,distance=2..] run particle minecraft:ash ~ ~ ~ 0.05 0.05 0.05 0.1 3 force
...
execute as @s at @s run function dvz:zombies/phantom/delirium_loop