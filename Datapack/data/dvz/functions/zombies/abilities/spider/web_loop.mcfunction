...
# File Name: web_loop
# Function Name: dvz:zombies/spider/web_loop
# File Purpose: Ray scanning for Spider's web ability.
...
execute unless entity @a[tag=temp_ray_origin,distance=..15] run return 0
execute if entity @a[tag=temp_hit] run return 0

# Teleports the ray slightly forward.
tp @s ^ ^ ^0.1

execute if entity @p[tag=temp_ray_origin,distance=2..] run particle minecraft:block minecraft:cobweb ~ ~ ~ 0 0 0 1 1 force
...
execute as @s at @s run function dvz:zombies/spider/web_loop