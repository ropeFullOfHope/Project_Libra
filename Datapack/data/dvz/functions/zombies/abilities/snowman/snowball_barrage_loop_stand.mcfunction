...
# File Name: snowball_barrage_loop_stand
# Function Name: dvz:zombies/snowman/snowball_barrage_loop_stand
...
execute store result score @s DVZ.deviationX.temp run random value -300..300
execute store result score @s DVZ.deviationY.temp run random value -75..75
execute store result score @s DVZ.deviationZ.temp run random value -300..300
...
execute store result entity @e[type=minecraft:snowball,tag=processing,limit=1,distance=..0.1,sort=nearest] Motion[1] double 0.0015 run scoreboard players get @s DVZ.deviationY.temp
...
execute if entity @s[scores={DVZ.snowballs.temp=1..}] run function dvz:zombies/snowman/snowball_barrage_loop_stand