...
execute as @e[type=minecraft:marker,tag=temp.ray,limit=1,sort=nearest] at @s run function ogvz:dwarves/water_wand_loop
...
execute as @s at @e[type=minecraft:marker,tag=temp.ray,limit=1,sort=nearest] if block ~ ~ ~ minecraft:water_cauldron run title @s actionbar [ \
  "", \
...
execute as @s at @e[type=minecraft:marker,tag=temp.ray,limit=1,sort=nearest] if block ~ ~ ~ minecraft:water_cauldron run playsound minecraft:item.bucket.empty block @a ~ ~ ~ 1 1

# Set a cooldown if a Cauldron was filled.
execute as @s at @e[type=minecraft:marker,tag=temp.ray,limit=1,sort=nearest] if block ~ ~ ~ minecraft:water_cauldron run scoreboard players set @s ogvz.alchemist.water_wand.cooldown.seconds 5
