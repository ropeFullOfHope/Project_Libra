...
scoreboard players add @s[tag=!builder,level=..99] ogvz.dwarf.mana_buildup 50
...
execute unless entity @e[type=minecraft:marker,tag=shrine,distance=..8] run scoreboard players remove @s[tag=!builder,level=101..,predicate=!ogvz:effect/hero_of_the_village] ogvz.dwarf.mana_buildup 10

# If a dwarf is near shrine, they gain 1 mana every 5 seconds (10 millimana every tick).
execute if entity @e[type=minecraft:marker,tag=shrine,distance=..8] run scoreboard players add @s ogvz.dwarf.mana_buildup 10