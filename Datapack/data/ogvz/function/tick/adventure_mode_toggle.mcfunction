...
execute as @s[gamemode=survival] unless entity @s[tag=!ogvz.adventure.lobby,tag=!ogvz.adventure.curse_of_limitation] run gamemode adventure @s

# If the player is in adventure mode and has no adventure tags, switch them to the survival mode.
execute as @s[gamemode=adventure] if entity @s[tag=!ogvz.adventure.lobby,tag=!ogvz.adventure.curse_of_limitation] run gamemode survival @s
